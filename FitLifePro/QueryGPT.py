...
openai.api_key = "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"
def QueryGPT(curUser, messages, modify):
    if len(messages) == 0:
...
Warm up: Short description of a good warm up based on the amount of time the person has.
...
Cooldown: Short description of a good cooldown based on the amount of time the person has.
...
Age- """
        ...
        if len(modify) > 3:
            modifyPlan = "Using the exact same instructions as above (do not alter any parts of the workout plan unless specifically told to do so to accomodate for the new instructions) revise the given workoutplan to accomodate the following instructions: "
            modifyPlan += modify
            messages.append({"role": "user", "content": modifyPlan})
            completion = openai.ChatCompletion.create(model="gpt-3.5-turbo", messages=messages)
            output = completion.choices[0].message.content
            messages.append({"role": "assistant", "content": output})
        else:
            modifyPlan = "return the exact same thing as the most recent workout plan output."
            messages.append({"role": "user", "content": modifyPlan})
            completion = openai.ChatCompletion.create(model="gpt-3.5-turbo", messages=messages)
            output = completion.choices[0].message.content
            messages.append({"role": "assistant", "content": output})
